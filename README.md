...
Если Gemini включён, после каждого нового уведомления бот автоматически
присылает **reply'ом** черновик ставки в стиле твоих прошлых заявок. Сбой ИИ или
отправки черновика не мешает боту пометить сам проект обработанным и перейти к
следующим уведомлениям. Под уведомлением также остаётся кнопка
**«✍️ Сгенерировать ответ»**, а под черновиком — **«🔄 Перегенерировать»**.
...
| `QUOTE_FILE`                 | `/data/quotes.json` | Отдельное атомарное хранилище зафиксированных часов, цен и сроков                    |
...
- `/start`, `/help` — приветствие и выбор категории: по кнопке на каждую
  ...
  **«⚙️ Настройки»**: добавление категории по `skill_id`, просмотр/замена JSON
  ...
  категориям.
- Под каждым уведомлением — **«✍️ Сгенерировать ответ»**; под каждым
  автоматическим или ручным черновиком — **«🔄 Перегенерировать»**.
...
app/
  ...
  ai/
    ...
    pricing.py                    # расчёт 12$/час и атомарное хранилище котировок
    prompt_store.py               # чтение/валидация редактируемого JSON промта
    prompts/
      system_prompt.md            # инструкции для модели
      scope_prompt.md             # выбор категории часов либо omit
...
Оценки хранятся отдельно в `QUOTE_FILE` (`/data/quotes.json`). Для конкретного
ТЗ Gemini выбирает только одну категорию объёма: 4, 8, 16, 24, 40 или 80 часов.
Программа считает одну фиксированную цену по ставке 12 USD/час, округляет её
вверх (UAH до 500, USD/EUR до 10, PLN до 50), а затем применяет точный бюджет
клиента как нижнюю границу. Если бюджета нет, цена выводится в UAH. При
перегенерации меняется только текст: сохранённые часы, цена и срок остаются
прежними. Для расплывчатого ТЗ цена и срок не выводятся.
